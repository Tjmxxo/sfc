...
<div class="page-container">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20"><span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a
            ...
            class="r">共有数据：<strong>${orders.size()}</strong> 条</span></div>
    <div class="mt-20">
        ...
        <table class="table table-border table-bordered table-bg">
            ...
            <c:forEach var="orders" items="${orders}">
                <tr class="text-c" class="changes">
                    ...
                    <td>${orders.userTbDTO.userAccount}</td>
                    <td>${orders.movieTbDTO.movieName}</td>
                    <td>${orders.hallTbDTO.hallName}</td>
                    <td>${orders.planTbDTO.planTime.toString()}</td>
                    <td>${orders.orderCreateTime.toString()}</td>
                    <c:choose>
                        <c:when test="${orders.orderPayTime.toString() == null}">
                            <td>暂未付款</td>
                        </c:when>
                        <c:when test="${orders.orderPayTime.toString() != null}">
                            <td>${orders.orderPayTime.toString()}</td>
                        </c:when>
                    </c:choose>
                    <td>${orders.orderPayPrice/100}</td>
                    <c:choose>
